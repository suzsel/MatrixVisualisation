...
ax.set_position([0.2, 0.3, 0.65, 0.8])
ax.set_xlim(-3, 3)
ax.set_ylim(-3, 3)
ax.set_zlim(-3, 3)
...
for i in range(len(x)):
    for j in range(len(y)):
        line, = ax.plot(X[:, i, j], Y[:, i, j], Z[:, i, j], 'gray', alpha=0.5)
        lines.append(line)
        line, = ax.plot(X[i, :, j], Y[i, :, j], Z[i, :, j], 'gray', alpha=0.5)
        lines.append(line)
        line, = ax.plot(X[i, j, :], Y[i, j, :], Z[i, j, :], 'gray', alpha=0.5)
...
def update(val):
    ...
    for i in range(len(x)):
        for j in range(len(y)):
            lines[k].set_data(transformed_points[0, :, i, j], transformed_points[1, :, i, j])
            lines[k].set_3d_properties(transformed_points[2, :, i, j])
            k += 1
            lines[k].set_data(transformed_points[0, i, :, j], transformed_points[1, i, :, j])
            lines[k].set_3d_properties(transformed_points[2, i, :, j])
            k += 1
...
for idx, (key, initial) in enumerate(initial_values_reversed):
    ax_slider = plt.axes([0.25, 0.17 + idx * slider_interval, slider_width, slider_height])
